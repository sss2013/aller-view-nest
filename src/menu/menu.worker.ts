...
export class MenuWorker {
    ...
    try {
      ...
      if (missItems.length > 0) {
        ...
        for (let i = 0; i < missItems.length; i++) {
          ...
          this.fetchAndUploadImage(normalized_text, newDish.id).catch((err) =>
  ...
  private async analyzeBatchWithGemini(menuNames: string[]): Promise<Array<{
    ...
    const defaultItem = { allergens: [], calorie_min: null, calorie_max: null, meat_ratio: 0, seafood_ratio: 0, vegetable_ratio: 0 };
...
각 음식에 대해:
1. 알레르겐: 반드시 아래 목록에서만 선택 (없으면 빈 배열)
   계란, 우유, 메밀, 땅콩, 대두, 밀, 고등어, 게, 새우, 돼지고기, 복숭아, 토마토, 아황산류, 호두, 닭고기, 쇠고기, 오징어, 굴, 전복, 홍합, 잣, 추출성분
2. 칼로리: 1인분 기준 최소/최대 추정 (kcal 정수)
3. 카테고리 비율: meat_ratio(육류), seafood_ratio(해산물), vegetable_ratio(채소/기타) — 합이 1.0

JSON 배열 형식으로만 응답 (순서 유지):
[
  { "name": "음식명", "allergens": ["새우"], "calorie_min": 400, "calorie_max": 600, "meat_ratio": 0.30, "seafood_ratio": 0.20, "vegetable_ratio": 0.50 }
]`;
